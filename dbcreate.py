...
def add_dream(content, title, description, author_id, author_name, tag):
    ...
    cursor.execute(
        """
        INSERT INTO dreams (dream_id, content, title, description, author_id, author_name, tag, upload_date)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?);
        """,
        (
            dream_id,
...
if __name__ == "__main__":
    # Add a new user (username, password, email, admin status)
    user_id = add_user("dreamer1", "password123", "dreamer1@example.com", admin=False)

    # Add a new dream (content, title, description, author_id, author_name, tag)
    add_dream(
        "I was flying over a city...",
        "Flying Dream",
        "A dream about flying high above a city",
        user_id,
        "dreamer1",
        "flying",
    )
